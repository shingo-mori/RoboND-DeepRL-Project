...
void ArmPlugin::Load(physics::ModelPtr _parent, sdf::ElementPtr /*_sdf*/) 
{
  ...
  this->cameraNode = gazebo::transport::NodePtr(new gazebo::transport::Node());
  this->cameraNode->Init();
	// Subscribe to camera topic
  this->cameraSub = this->cameraNode->Subscribe("/gazebo/arm_world/camera/link/camera/image", &ArmPlugin::onCameraMsg, this);

	// Create our node for collision detection
  this->collisionNode = gazebo::transport::NodePtr(new gazebo::transport::Node());
  this->collisionNode->Init();
  // Subscribe to prop collision topic
  this->collisionSub = this->collisionNode->Subscribe("/gazebo/arm_world/tube/tube_link/my_contact", &ArmPlugin::onCollisionMsg, this);
...
bool ArmPlugin::createAgent()
{
	...
	agent = dqnAgent::Create(
	    INPUT_WIDTH, INPUT_HEIGHT, INPUT_CHANNELS, DOF * 2,
...
void ArmPlugin::onCollisionMsg(ConstContactsPtr &contacts)
{
	...
	for (unsigned int i = 0; i < contacts->contact_size(); ++i)
	{
		...
		// If there is collision between the arm and object, then issue learning reward
...
bool ArmPlugin::updateAgent()
{
	...
	float coeff = (action % 2) == 0 ? 1.0 : -1.0;
	float velocity = vel[action/2] + coeff * actionVelDelta;
	...
	float coeff = (action % 2) == 0 ? 1.0 : -1.0;
	float joint = ref[action/2] + coeff * actionJointDelta;
	...
	if( hadNewState && !newReward )
	{
		...
		if( checkGroundContact )
		{
			...
			if(DEBUG)
			{
				...
				printf("minz: %f, maxz: %f", gripBBox.min.z, gripBBox.max.z);
			}
